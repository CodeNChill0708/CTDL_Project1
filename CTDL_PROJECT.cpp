...
const int pink = 13;
const string so_du_ban_dau_mac_dinh = "50000";
const string don_vi_tien_te_mac_dinh = "VND";
const int max_nhap_sai = 3;
...
private:
    ...
    void rut_tien(User* user, User* head)
    {
        ...
        {
            cout << "So du khong du de rut tien (duoi 100k)" << endl;
            myfile.close();
            system("pause");
            user->menu_user(user, head);
        }
        ...
        {
            cout << "So tien can rut phai lon hon 50, la boi cua 50 va duoi " << stoi(money) - 50000 << "!" << endl;
    ...
    void chuyen_tien(User* user, User* head)
    {
        ...
        if (stoi(sdkd) < 100000)
        {
            cout << "So du tai khoan khong du de thuc hien giao dich nay!" << endl;
            system("pause");
            user->menu_user(user, head);
        }
        else {
            ...
            {
                cout << "Tai khoan thu huong phai gom 14 chu so, khong duoc chuyen cho chinh minh, nhap lai (0:exit): "; getline(cin >> ws, tkth);
                if (tkth == "0")
                {
                    user->menu_user(user, head);
                }
            ...
            while (check != NULL)
            {
                if (check->user_name == tkth)
                {
                    ...
                    {
                        cout << "So tien can chuyen phai lon hon 50k, boi cua 50k va duoi " << stoi(sdkd) - 50000 << " nhap lai: ";
            ...
            cout << "Khong tim thay ten tai khoan thu huong, vui long thu lai!" << endl;
            source_file.close();
            user->chuyen_tien(user, head);
        }
    }
    ...
    void doi_mat_khau(User* user, User* head)
    {
        var mkm;
        var nlmkm;
        cout << "Nhap mat khau moi (0:exit): "; user->take_input_password(mkm);
        if (mkm == "0")
        {
            user->menu_user(user, head);
        }
        ...
        {
            mkm = "";
            cout << "Mat khau phai gom 6 chu so, khac mat khau cu va khong duoc qua don gian, thu lai (0:exit): "; user->take_input_password(mkm);
            if (mkm == "0")
            {
                user->menu_user(user, head);
            }
        ...
        if (mkm != nlmkm)
        {
            cout << "Mat khau khong trung khop, vui long thu lai!" << endl;
            user->doi_mat_khau(user, head);
        }
        else {
            user->password = mkm;
    ...
    void dang_nhap_user(User* head)
    {
        ...
        while (user != NULL)
        {
            while (user->user_name == usn && user->password != pin)
            {
                ...
                cout << endl;
                cout << "Sai ma pin, ban con " << max_nhap_sai - user->nhap_sai << " lan thu: ";
                pin = "";
            ...
            if (user->user_name == usn && user->password == pin)
            {
                ...
                {
                    cout << endl;
                    cout << "Lan dau tien dang nhap, hay doi mat khau!" << endl;
                    user->doi_mat_khau(user, head);
                    head->update_file_the_tu(head);
                    return;
                }
...
private:
    ...
    void them_tai_khoan(Admin* user, Admin* head)
    {
        ...
        myfile << name << endl << so_du_ban_dau_mac_dinh << endl << don_vi_tien_te_mac_dinh << endl;
        myfile.close();
    ...
    void xoa_tai_khoan(Admin* heada, Admin* head)
    {
        ...
        while (true)
        {
            cout << "Nhap ID tai khoan can xoa (0:exit): "; getline(cin >> ws, id);
            if (id == "0")
            {
            ...
            while (id.length() != 14)
            {
                cout << "ID tai khoan phai bao gom 14 chu so, nhap lai (0:exit): "; getline(cin >> ws, id);
            }
    ...
    {
        cout << "Nhap tai khoan can mo khoa (0:exit): ";
